...
export const customCursor = () => {
	let myCursor = document.querySelectorAll('.mouse-cursor'),
		...
		extraTopbar = document.querySelector('.extraTopbar '),
		...
		if (document.body) {
			...
			(window.onmousemove = function (s) {
				...
				document.body.addEventListener(
					'mouseenter',
					// "a,.extraTopbar .trigger, .cursor-pointer",
					function () {
						...
						extraTopbar && mouseEvent(extraTopbar);
...
// Active Section
export const sectionActiveFunc = (value) => {
	const sections = document.querySelectorAll('.section');
	const menuLists = document.querySelectorAll('.transition_link li');
	sections.forEach((section) => {
		let id = section.getAttribute('id');
		if (id == value) {
			section.className = 'section active wow animated fadeInUp';
		} else {
			section.className = 'section hidden animated';
		}
	});
};
...
// custom pagination in swiper slider

export const swiperSliderCustomSlider = (current, total, currentCls, totalCls, allCls) => {
	var scale, translateX;
	var progressDOM = document.querySelector('.swiper_progress');
